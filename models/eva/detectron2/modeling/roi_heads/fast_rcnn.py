...
def fast_rcnn_inference(
    ...
    result_per_image = [
        fast_rcnn_inference_single_image(
            ...
            keep_all=keep_all_before_merge, logits=logit_per_image
        )
        for scores_per_image, boxes_per_image, image_shape, logit_per_image in zip(scores, boxes, image_shapes, logits)
    ]
...
def fast_rcnn_inference_single_image(
    ...
    if use_soft_nms:
        warnings.warn('SoftNMS does not return logit yet')
        if not class_wise:
    ...
    if logits is not None:
        # process logits
        # while True:
            # _, logits_cls = logits.max(dim=1)
            # unmatch_idx = torch.where(filter_inds[:, 1] != logits_cls)[0]
            # if len(unmatch_idx) == 0:
            #     break
            # cls_idx = torch.tensor([logits_cls[x] for x in unmatch_idx], device=logits.device, dtype=torch.long)
            # logits[unmatch_idx, cls_idx] = logits[unmatch_idx].min(dim=1)[0]
        result.logits = logits
...
class FastRCNNOutputLayers(nn.Module):
    ...
    def losses(self, predictions, proposals):
        ...
        gt_classes_ori = gt_classes
        # changes start here
        # parse weights if available
        if self.is_logit:
            gt_logits = (
                cat([p.gt_logits for p in proposals], dim=0) if len(proposals) else torch.empty(
                    (0, self.num_classes + 1))
            )
            assert (gt_logits.max(1)[1] == gt_classes).all()
            gt_classes = gt_logits
            # print(gt_classes, gt_classes.shape)

        has_weights = False
        gt_weights = None
        if len(proposals):
            if proposals[0].has('weights'):
                has_weights = True
                gt_weights = (
                    cat([p.weights for p in proposals], dim=0) if len(proposals) else torch.empty(0)
                )
                # print(gt_weights)
        ...
        if self.use_sigmoid_ce:
            loss_cls = self.sigmoid_cross_entropy_loss(scores, gt_classes, gt_weights)
        else:
            if has_weights:
                loss_cls = torch.mean(cross_entropy(scores, gt_classes, reduction="none") * gt_weights)
            else:
        ...
        losses = {
            ...
            "loss_box_reg": self.box_reg_loss(
                proposal_boxes, gt_boxes, proposal_deltas, gt_classes_ori, gt_weights
            ),
        }
    ...
    # with slight modifications
    def sigmoid_cross_entropy_loss(self, pred_class_logits, gt_classes, gt_weights=None):
        """
        Args:
        ...
        if self.use_fed_loss:
            ...
            weight = gt_weights if gt_weights is not None else 1

        loss = torch.sum(cls_loss * weight) / N
        return loss

    def box_reg_loss(self, proposal_boxes, gt_boxes, pred_deltas, gt_classes, gt_weights=None):
        """
        Args:
        ...
        loss_box_reg = _dense_box_regression_loss(
            ...
            self.smooth_l1_beta,
            gt_weights[fg_inds] if gt_weights is not None else None
        )
    ...
    def inference(self, predictions: Tuple[torch.Tensor, torch.Tensor], proposals: List[Instances]):
        ...
        scores, _ = self.predict_probs(predictions, proposals)
