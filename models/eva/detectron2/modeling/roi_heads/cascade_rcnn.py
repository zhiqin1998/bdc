...
class CascadeROIHeads(StandardROIHeads):
    ...
    def _forward_box(self, features, proposals, targets=None, keep_all_before_merge=False):
        ...
        if self.training:
            ...
            scores = [
                ...
                for scores_per_image in zip(*[x[0] for x in scores_per_stage])
            ]
            logits = [
                sum(list(scores_per_image)) * (1.0 / self.num_cascade_stages)
                for scores_per_image in zip(*[x[1] for x in scores_per_stage])
            ]
    ...
    def _match_and_label_boxes(self, proposals, stage, targets):
        ...
        for proposals_per_image, targets_per_image in zip(proposals, targets):
            ...
            # weights if needed
            has_weights = targets_per_image.has('weights')
            has_logits = targets_per_image.has('gt_logits')
            if len(targets_per_image) > 0:
                ...
                if has_weights:
                    gt_weights = targets_per_image.weights[matched_idxs]
                if has_logits:
                    gt_logits = targets_per_image.gt_logits[matched_idxs]
                    gt_logits[proposal_labels == 0] = torch.nn.functional.one_hot(gt_classes[proposal_labels == 0], self.num_classes + 1).float()
            else:
                ...
                if has_weights:
                    gt_weights = torch.ones_like(matched_idxs).float()
                if has_logits:
                    gt_logits = torch.nn.functional.one_hot(gt_classes, self.num_classes + 1).float()
            proposals_per_image.gt_classes = gt_classes
            proposals_per_image.gt_boxes = gt_boxes
            if has_weights:
                proposals_per_image.weights = gt_weights
            if has_logits:
                proposals_per_image.gt_logits = gt_logits
