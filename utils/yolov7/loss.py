...
class ComputeLossOTAWithLogits(ComputeLossOTA):
    """ComputeLossOTA Class but loss is computed with logits instead of class id (for BDC)"""
    # when target is logits instead of class id
    def __call__(self, p, targets, imgs, weights=None):  # predictions, targets, model
        device = targets.device
        lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
        bs, as_, gjs, gis, targets, anchors, weights = self.build_targets(p, targets, imgs, weights)
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                pwh = (ps[:, 2:4].sigmoid() * 2) ** 2 * anchors[i]
                pbox = torch.cat((pxy, pwh), 1)  # predicted box
                selected_tbox = targets[i][:, -4:] * pre_gen_gains[i]
                selected_tbox[:, :2] -= grid
                iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False, CIoU=True)  # iou(prediction, target)
                lbox += ((1.0 - iou) * selected_weights.unsqueeze(1)).mean()  # iou loss
                ...
                #     lcls += self.BCEcls(ps[:, 5:], t)  # BCE
                if self.nc > 1:  # cls loss (only if multiple classes)
                    t = targets[i][:, 1:-4].to(device)
                    if self.weighted:
                        lcls += torch.mean(self.BCEcls(ps[:, 5:], t) * selected_weights.unsqueeze(1))
                    else:
                        lcls += self.BCEcls(ps[:, 5:], t)  # BCE
    ...
    def build_targets(self, p, targets, imgs, weights=None):
        ...
        # indices, anch = self.find_positive(p, targets)
        # class is not used here so we can just create dummy for this if logits is passed in
        indices, anch = self.find_3_positive(p, targets[:, [0, 1, -4, -3, -2, -1]])
        # indices, anch = self.find_4_positive(p, targets)
        # indices, anch = self.find_5_positive(p, targets)
        # indices, anch = self.find_9_positive(p, targets)
        device = torch.device(targets.device)
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            txywh = this_target[:, -4:] * imgs[batch_idx].shape[1]
            txyxy = xywh2xyxy(txywh)
...
if __name__ == '__main__':
    ...
    from utils.yolov7.general import transform_to_yolov7_targets
    from utils.general import init_seeds

    with open('./data/voc_2007_ann10_full.yaml') as f:
        data_dict = yaml.load(f, Loader=yaml.SafeLoader)  # data dict
    init_seeds(1)
    aggregator = MVAggregator(data_dict['n_annotator'])
    ...
    dataloader = DataLoader(dataset, batch_size=2, num_workers=1, pin_memory=True,
                            collate_fn=collate_fn, shuffle=False)
    with open('models/yolov7/config/hyp.scratch.p5.yaml') as f:
        hyp = yaml.load(f, Loader=yaml.SafeLoader)
    model = Model('./models/yolov7/config/yolov7.yaml', ch=3, nc=21, anchors=hyp.get('anchors'))
    model.hyp = hyp
    model.nc = 21  # attach number of classes to model
    model.hyp = hyp  # attach hyperparameters to model
    model.gr = 1.0  # iou loss ratio (obj_loss = 1.0 or iou)
    loss = ComputeLossOTAWithLogits(model, weighted=True)
    ori_loss = ComputeLossOTA(model)
    weighted_ori_loss = ComputeLossOTA(model, weighted=True)

    imgs, ori_targets, paths, _, clean_targets = next(iter(dataloader))
    targets = transform_to_yolov7_targets(ori_targets, logits=False)
    pred = model(imgs)
    weights = torch.ones(len(targets))

    print(ori_loss(pred, targets, imgs))
    print(weighted_ori_loss(pred, targets, imgs, weights=weights))

    # print(loss(pred, targets, imgs))
    #
    logit_targets = [np.concatenate((t[:, :4], np.eye(21)[t[:, 4].astype(int)]), axis=1) for t in ori_targets]
    logit_targets = transform_to_yolov7_targets(logit_targets, logits=True)
    # print(logit_targets)
    print(loss(pred, logit_targets, imgs, weights=weights))

    weights = torch.rand(len(targets))
    print(weighted_ori_loss(pred, targets, imgs, weights=weights))
    print(loss(pred, logit_targets, imgs, weights=weights))
