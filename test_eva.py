...
from utils.eva.general import transform_to_eva_targets, collate_fn
...
def test(data,
    ...
    if not training:
        ...
        test_dataset = CleanDataset(img_dir=data['image_dir'], annotations_path=data[task],
                                    image_size=opt.img_size, normalize_box=False)
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        targets = transform_to_faster_rcnn_targets(targets)  # lazy hack
        ...
        with EventStorage(0) as storage:
            with torch.autocast(device_type=device.type, dtype=torch.float16, enabled=half):
                with torch.no_grad():
                    ...
                    model.eval()
                    detections = model(datas)
                    t0 += time_synchronized() - t

                    # loss += torch.stack((loss_dict['loss_rpn_loc'], loss_dict['loss_box_reg_stage2'],
                    #                      loss_dict['loss_rpn_cls'], loss_dict['loss_cls_stage2'])).detach()

                    # convert to proper format list of tensor [xyxy, conf, cls]
                    out = []
                    for det in detections:
                        det = det['instances']
                        temp = torch.zeros((len(det), 6), device=device)
                        temp[:, :4] = det.pred_boxes.tensor
                        temp[:, 4] = det.scores
                        temp[:, 5] = det.pred_classes
                        out.append(temp)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--img-size', type=int, default=1024, help='test image sizes')
